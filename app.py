...
def translate_and_phoneticize(text):
    ...
    translation_response = client.chat.completions.create(
        ...
        messages=[
            {"role": "system", "content": "You are a helpful assistant that translates Korean text to English. For short sentences, provide alternative expressions separated by '|||'. For long sentences, provide only one translation."},
    ...
    for segment in segments:
        phonetic_response = client.chat.completions.create(
            model="gpt-4o-mini-2024-07-18",
            messages=[
                {"role": "system", "content": "당신은 영어 발음을 한글로 정확하게 표기하는 전문가입니다."},
                {"role": "user", "content": f"다음 영어 문장을 한국어 문자(한글)로 음사해 주세요. 영어 원어민의 자연스러운 발음을 최대한 반영하여 표기해 주세요. 오직 한글 음사만 작은따옴표 안에 제공해 주세요: '{segment}'"}
            ],
            temperature=0.3,
        )
